        ...
        return (
            <Card style={{ padding: 16 }}>
                ...
                {!this.state.canRequestPayments && <Button size="large" type="submit" variant="contained" color="secondary" id="signup" style={{ width: '180px', height: '50px', color: 'white', marginTop: '25px' }} onClick={() => {
                    var win = window.open(`https://connect.stripe.com/express/oauth/authorize?redirect_uri=https://app.travel-left.com/account/travelerPayments/verify_connect_account&${process.env.REACT_APP_STRIPE_CONNECT_CLIENT_ID}&state=${uuid()}`, '_blank');
                    win.focus()
                }}>
                    Verify account
                </Button>}
