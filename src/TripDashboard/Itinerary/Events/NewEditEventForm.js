...
export default class NewEditEventForm extends Component {
    state = {
        ...
        docs: [],
        selectedDocs: this.props.event.documents,
        startTime: moment(this.props.event.dtStart, ["h:mm A"]).tz(this.props.event.tzStart).format("HH:mm"), //needs to be converted from local timezone to designated timezone
        endTime: moment(this.props.event.dtEnd, ["h:mm A"]).tz(this.props.event.tzStart).format("HH:mm"),
        timezone: this.props.event.tzStart.replace(' ', '_'),
    ...
    handleDocumentsChange = documents => {
        if (documents) {
            ...
            this.setState({
                selectedDocs: this.state.docs.filter(doc => documents.includes(doc._id))
            })
        } else {
            this.setState({
                selectedDocs: []
    ...
    closeModal = () => {
        this.setState({
            ...
            docs: [],
            selectedDocs: this.props.event.documents,
            startTime: moment(this.props.event.dtStart, ["h:mm A"]).format("HH:mm"),
            endTime: moment(this.props.event.dtEnd, ["h:mm A"]).format("HH:mm"),
            timezone: this.props.event.tzStart.replace(' ', '_'),
            description: this.props.event.description,
            location: this.props.event.address
        })
            ...
            this.props.step === 3 ? (
                <div>
                    ...
                    <Select
                        isMulti
                        name="type"
                        options={this.props.docs.map(doc => (
                            {
                                label: doc.name,
                                value: doc._id
                            }
                        ))}
                        value={this.props.selectedDocs.map(doc => (
                            {
                                label: doc.name,
                                value: doc._id
                            }
